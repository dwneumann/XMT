...
#*   $Version:$
#*   Package	: xmt_defect
#*   Purpose	: Person class.  Used by defectDB.pl.
...
#************************************************************************
# new($oid): instantiate a new Person object. If $oid is
...
sub delete
{
    ...
    if (defined($self->{oid}))
    {
	$stmt = "delete from Person where oid = $self->{oid}";
	$sth = $self->{dbh}->prepare($stmt) or return undefErr;
	$sth->execute or return undefErr;
    }
}
...
sub _fetch
{
    ...
    $stmt = "select oid,* from Persons 
	    where uname = '$self->{uname}'";
    $sth = $self->{dbh}->prepare($stmt) or return undefErr;
    $sth->execute or return undefErr;
