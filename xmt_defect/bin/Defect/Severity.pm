...
#*   $Version:$
#*   Package	: xmt_defect
#*   Purpose	: Severity class.  Used by defectDB.pl.
...
#************************************************************************
# new($oid): instantiate a new Severity object. If $oid is supplied,
# the object is associated with the DefectDB record with the same
# oid. The DefectDB is accessed through DBI
# connection $DefectDB::dbh. Returns true if the object is
...
sub update
{
    ...
    if (defined($self->{oid}))
    {
	$stmt = "update Severities set
		    sev		= '$self->{sev}'
		    where oid	= $self->{oid} ";
    }
    else
    {
	$stmt = "insert into Severities 
		    values ( '$self->{sev}' )";
    }
...
sub delete
{
    ...
    if (defined($self->{oid}))
    {
	$stmt = "delete from Severities where oid = $self->{oid}";
	$sth = $self->{dbh}->prepare($stmt) or return undefErr;
	$sth->execute or return undefErr;
    }
}
...
sub _fetch
{
    ...
    $stmt = "select oid,* from Severities 
	    where sev = $self->{sev}";
    $sth = $self->{dbh}->prepare($stmt) or return undefErr;
    $sth->execute or return undefErr;
    $rc = $sth->bind_columns(undef, \$self->{sev});
