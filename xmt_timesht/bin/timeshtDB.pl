...
#*   $Version:$
#*   Package	: xmt_timesht
#*   Synopsis	: timeshtDB.pl
#*   Purpose	: A library of functions to access the timesheet database.
...
sub connect
{
    ...
    $TimeshtDB::dbh	= DBI->connect( "dbi:Pg:$dbname", '', '', ) || 
...
sub query
{
    my ($stmt) = @_;
    my ($hashref);
    $TimeshtDB::sth = $TimeshtDB::dbh->prepare( $stmt) || dbdie( "$DBI::errstr" );
    $TimeshtDB::sth->execute || dbdie( "$DBI::errstr" );
    while ( $hashref = $TimeshtDB::sth->fetchrow_hashref )
    {
    ...
    #********************************************************************
    $TimeshtDB::sth->finish; 
}

#************************************************************************
# loadquery($fn): Returns the contents of file $fn into a buffer 
# If no $fn is specified the default is $XMTTIMESHT/proto/queries/default
#************************************************************************
sub loadquery
{
    my ($fn, $buf) = @_;
    ...
    $fn = sprintf("%s/proto/queries/default", $ENV{'XMTTIMESHT'}) 
    	if !defined($fn);
...
# the file $fn. 
# If no $fn is specified the default is $XMTTIMESHT/proto/reports/default
#************************************************************************
sub format_report
{
    my ($fn, $buf) = @_;

    $fn = sprintf("%s/proto/reports/default", $ENV{'XMTTIMESHT'}) 
    	if !defined($fn);
    ...
    {
	open( TMPL, "<$fn" ) || TimeshtDB::dbdie( "$fn: $!" );
	undef $/;
	$buf = <TMPL>;
	close( TMPL ) || TimeshtDB::dbdie( "$fn: $!" );
	eval $buf;
    }
}
...
{
    my ($msg) = @_;
    $TimeshtDB::dbh->disconnect	if ( defined $TimeshtDB::dbh );
    undef $TimeshtDB::dbh;
    undef $TimeshtDB::sth;
