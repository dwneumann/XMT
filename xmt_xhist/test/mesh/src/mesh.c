...
/* xhist instrument TRUE */
    ...
    globals.gl_pkts_to_send	= 10;	_XH_ADD( 2677, 123 );
    globals.gl_num_hops		= 200;_XH_ADD( 2677, 124 );
    signal(SIGINT, test_complete);	/* print stats & exit upon receipt of SIGINT	*/_XH_ADD( 2677, 125 );
    BACKOUT_IF(argc < 2, "usage: mesh <my (0-based) node index> <port#> <port#> <port#> ...");_XH_ADD( 2677, 126 );
    globals.gl_my_nodenum 	= atoi(argv[1]);_XH_ADD( 2677, 127 );
    for (i = 2; i < argc; ++i)
    {
        globals.gl_nodes[i-2] = atoi(argv[i]);_XH_ADD( 2677, 130 );
    }
    globals.gl_num_nodes = argc-2;_XH_ADD( 2677, 132 );

    BACKOUT_IF((sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0, "socket");_XH_ADD( 2677, 134 );
    memset(&node, 0, sizeof(struct sockaddr_in));_XH_ADD( 2677, 135 );
    node.sin_family	= AF_INET;_XH_ADD( 2677, 136 );
    node.sin_addr.s_addr = inet_addr("127.0.0.1");_XH_ADD( 2677, 137 );
    node.sin_port = htons(globals.gl_nodes[globals.gl_my_nodenum]);_XH_ADD( 2677, 138 );
    BACKOUT_IF(bind(sock, (struct sockaddr *)&node, sizeof(node)) < 0, "bind"); _XH_ADD( 2677, 139 );

    /* wait until the node I'm going to send to has started */
    next_node = globals.gl_nodes[(globals.gl_my_nodenum+1)%globals.gl_num_nodes];_XH_ADD( 2677, 142 );
    while ( ! is_listening(next_node) )
    {
	sleep(1);_XH_ADD( 2677, 145 );
    }

    /* @send my initial messages to my first-listed neighbour	*/
    for (i = 0; i < globals.gl_pkts_to_send; ++i)
    {
	node.sin_port = htons(next_node);_XH_ADD( 2677, 151 );
	init_payload(&payload, globals.gl_nodes[globals.gl_my_nodenum], next_node, globals.gl_num_hops);_XH_ADD( 2677, 152 );
	sendto(sock, &payload, sizeof(payload), 0, (struct sockaddr *) &node, sizeof(node));_XH_ADD( 2677, 153 );
    }

    /* now just forward or ack messges received from others */
    next_node = globals.gl_my_nodenum;_XH_ADD( 2677, 157 );
    globals.gl_pkts_returned = 0;_XH_ADD( 2677, 158 );
    while (1)
    {
	nbytes = recvfrom(sock, &payload, sizeof(payload), 0, NULL, NULL);_XH_ADD( 2677, 161 );
	...
	{
	    globals.gl_pkts_returned++;_XH_ADD( 2677, 172 );
	    globals.gl_total_time += round_trip_ms(payload.pkt_sent_at);_XH_ADD( 2677, 173 );

#ifdef DIAGNOSTICS
	    if ( diagnosing(DIAG_MESH_RX) )
	    {
		fprintf( DIAG_FD, "%s:%d\t: ", __FILE__, __LINE__ ); _XH_ADD( 2677, 178 );
		fprintf( DIAG_FD, "%2d : RECV'd ACK after %2d hops in %ld ms\n", 
		    globals.gl_nodes[globals.gl_my_nodenum], 
		    payload.pkt_num_fwds, 
		    round_trip_ms(payload.pkt_sent_at));_XH_ADD( 2677, 182 );
	    }
#endif 
	}
	...
	{
	    node.sin_port = htons(payload.pkt_src);_XH_ADD( 2677, 190 );
	    payload.pkt_num_fwds++;_XH_ADD( 2677, 191 );
	    payload.pkt_ttl--;_XH_ADD( 2677, 192 );
	    sendto(sock, &payload, sizeof(payload), 0, 
		(struct sockaddr *) &node, sizeof(node));_XH_ADD( 2677, 194 );
	}

	/* forward message one more hop to someone else */
	else	
	{
	    next_node	  = ((next_node+1) % globals.gl_num_nodes); _XH_ADD( 2677, 200 );
	    node.sin_port = htons(globals.gl_nodes[next_node]);_XH_ADD( 2677, 201 );
	    payload.pkt_num_fwds++;_XH_ADD( 2677, 202 );
	    payload.pkt_ttl--;_XH_ADD( 2677, 203 );
	    sendto(sock, &payload, sizeof(payload), 0, 
		(struct sockaddr *) &node, sizeof(node));_XH_ADD( 2677, 205 );
...
void init_payload(packet_t *payload, int from, int to, int ttl)
{
    memset(payload, 0, sizeof(packet_t));_XH_ADD( 2677, 233 );
    payload->pkt_src 	= from;_XH_ADD( 2677, 234 );
    payload->pkt_to	= to;_XH_ADD( 2677, 235 );
    payload->pkt_ttl	= ttl;_XH_ADD( 2677, 236 );
    gettimeofday(&(payload->pkt_sent_at), NULL);_XH_ADD( 2677, 237 );
}
...
long round_trip_ms(struct timeval from) 
{
    ...
    gettimeofday(&to, NULL);_XH_ADD( 2677, 259 );
...
int is_listening(int port) 
{
    ...
    sprintf(str, "netstat -lnu | grep %d", port);_XH_ADD( 2677, 285 );
    BACKOUT_IF((fp = popen(str, "r")) == NULL, "popen");_XH_ADD( 2677, 286 );
    if ( fgets(str, sizeof(str)-1, fp) != NULL )
    {
	status = 1;_XH_ADD( 2677, 289 );
    }
    fclose(fp); _XH_ADD( 2677, 291 );
    return(status);
}
...
void test_complete()
{
	...
	(int) (globals.gl_total_time/(1 + globals.gl_pkts_returned)));_XH_ADD( 2677, 316 );
    exit( (globals.gl_pkts_returned == globals.gl_pkts_to_send) ? TEST_PASS : TEST_FAIL );_XH_ADD( 2677, 317 );
}
