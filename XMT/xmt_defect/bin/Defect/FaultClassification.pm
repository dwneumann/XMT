...
#*   $Version:$
#*   Package	: xmt_defect
#*   Purpose	: FaultClassification class.  Used by defectDB.pl.
...
package Defect::FaultClassification;
use Carp;
use DBI;
...
#************************************************************************
# new($oid): instantiate a new FaultClassification
# object. If $oid is supplied, the object is
# associated with the DefectDB record with the same oid.  The DefectDB is
# accessed through DBI connection $DefectDB::dbh. Returns true if the
# object is successfully instantiated, and undef otherwise.
#************************************************************************
sub new 
{
    my ($class, $oid) = @_;
...
sub update
{
    ...
    if (defined($self->{oid}))
    {
	$stmt = "update FaultClassifications set
		    phase	= '$self->{phase}',
		    type	= '$self->{type}',
		    reason	= '$self->{reason}'
		    where oid	= $self->{oid}";
    ...
    {
	$stmt = "insert into FaultClassifications 
		    values (
		    '$self->{phase}',
		    '$self->{type}',
		    '$self->{reason}')";
...
sub delete
{
    ...
    if (defined($self->{oid}))
    {
	$stmt = "delete from FaultClassification where oid = $self->{oid}";
	$sth = $self->{dbh}->prepare($stmt) or return undefErr;
	$sth->execute or return undefErr;
    }
}
...
sub _fetch
{
    ...
    $stmt = "select oid,* from FaultClassifications 
	    where phase = '$self->{phase}' and type = '$self->{type}'
